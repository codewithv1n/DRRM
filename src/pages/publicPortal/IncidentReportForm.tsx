...
import { AlertCircle, Camera, CheckCircle2, MapPin, Phone, User, Loader2, Search, Flame, Droplets, Activity, AlertTriangle, Navigation, ArrowLeft, Upload, Mountain, HelpCircle } from 'lucide-react';
...
export default function IncidentReportForm() {
  ...
  const handleInitialSubmit = (e: React.FormEvent) => {
    ...
    const submitIncident = () => {
      ...
      addIncident({
          ...formData,
          type: finalType as EmergencyType,
                ...
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {[
                            ...
                            { type: 'Earthquake', icon: Mountain, color: 'text-stone-600', bg: 'bg-stone-100', border: 'border-stone-300' },
                            { type: 'Road Obstruction', icon: AlertTriangle, color: 'text-amber-500', bg: 'bg-amber-50', border: 'border-amber-200' },
                            { type: 'Other', icon: HelpCircle, color: 'text-slate-500', bg: 'bg-slate-100', border: 'border-slate-300' },
                        ].map((em) => {
                    ...
                    {formData.type === 'Other' && (
                        <div className="mt-4 animate-fade-in-up">
                            ...
                            <input
                                ...
                                className="block w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-primary/20 focus:border-primary text-sm transition-colors"
                                placeholder="e.g., Gas Leak, Animal Rescue..."
                            />
                        </div>
                    )}
                </div>
