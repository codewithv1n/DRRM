...
export default function ResponderHazardMapPage() {
  ...
  const activeIncidentsCount = dbIncidents.filter(i => 
      ...
      <div className="animate-fade-in space-y-8">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-slate-500 mt-1">Real-time Quezon City hazard data</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={refetch}
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors disabled:opacity-50 cursor-pointer"
            >
        ...
        {error && (
          <div className="flex items-center gap-2 p-3 bg-red-50 text-red-700 rounded-xl border border-red-200 text-sm">
            ...
            <span>{error}</span>
            <button onClick={refetch} className="ml-auto text-xs font-bold underline hover:no-underline cursor-pointer">Retry</button>
          </div>
        )}
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mb-4">Quezon City area data from Open-Meteo Weather API.</p>
