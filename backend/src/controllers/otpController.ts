...
export const sendOtp = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (type === 'login') {
      ...
      res.status(400).json({ error: 'Invalid OTP request type.' });
      return;
    }

    const senderName = type === 'login' ? 'GovServe DRRM Login OTP' : type === 'forgot_password' ? 'GovServe DRRM Password Reset' : type === 'signup' ? 'GovServe DRRM Registration' : 'GovServe DRRM OTP';
